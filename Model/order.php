...
<?php
    class order{
        ...
        public function insert_order_admin($data){
            ...
            public function update_order_admin($data){ 
                ...
                `zipcode`='$zipcode',`name_product`='$nameProduct',`ship_time`='$ship_time', `quantity`='$quantity',`service`='$service',`delivery_time`='$delivery_time',`trangthai`='$status',`length`='$length',`width`='$width',`height`='$height',`weight`='$weight',`price`='$price',`payment` = '$payment' WHERE `idOrder` = '$idOrder'";
                $result = $this->db->update($query);
                if($result){
                    $alert=  '<div class="alert alert-success alert-dismissible fade show" role="alert">
                    Cập nhật đơn hàng thành công!.
        ...
        public function delete_label($id, $filename){ // sửa đổi tham số của hàm ở đây
            ...
            $query = "DELETE FROM pdf_data WHERE `idOrder` = '$id' and `filename` = '$filename'";
    ...
    public function show_order_by_customer($name_user){
        $query = "SELECT * FROM oder WHERE `name_user` = '$name_user' and `status` = 1 order by `time` DESC"; 
        $result = $this->db->select($query);
        return $result;
    }
    ...
    public function show_congno($name_user){
        $query = "SELECT 
        SUM(CASE WHEN `payment` = 1 AND `status` = 1 THEN price ELSE 0 END) AS da_thanh_toan,
        SUM(CASE WHEN `payment` <> 1 AND `status` = 1 THEN price ELSE 0 END) AS chua_thanh_toan,
