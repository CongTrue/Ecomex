...
function compareTextInPdf($pdfFile, $startCharacter, $endCharacter, $con) {
    ...
    for ($page = 1; $page <= $totalPages; $page++) {
        ...
        if ($startPos !== false && $endPos !== false && $endPos > $startPos) {
            ...
            $extractedText = substr($text, $startPos , $endPos - $startPos+3);
            // Tạo tên tệp PDF mới dựa trên số trang và phần của văn bản được trích xuất
            $newPdfFileName = $extractedText . $page . '.pdf';
            // Lưu tệp PDF mới
            $pdf->Output('./pdf/' . $newPdfFileName, 'F');
            $time = date('Y-m-d H:i:s');
            $name_user = $_SESSION['name'];
            $insertquery = "INSERT INTO pdf_data(`idOrder`,`filename`,`time`,`name_user`) VALUES('$extractedText','$newPdfFileName','$time','$name_user')";
